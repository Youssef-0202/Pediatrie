...
@Component({
  selector: 'app-analyse-medicale-edit-infermier',
  templateUrl: './analyse-medicale-edit-infermier.component.html'
})
export class AnalyseMedicaleEditInfermierComponent implements OnInit {
    ...
    constructor(private service: AnalyseMedicaleInfermierService , private consultationService: ConsultationInfermierService, private epreuveService: EpreuveInfermierService, @Inject(PLATFORM_ID) private platformId?) {
    ...
    public prepareEdit() {
        this.item.dateAnalyseMedicale = this.service.format(this.item.dateAnalyseMedicale);
    }
 ...
 public edit(): void {
   ...
   public async openCreateEpreuve(epreuve: string) {
        const isPermistted = await this.roleService.isPermitted('Epreuve', 'edit');
        if (isPermistted) {
