...
@Component({
  selector: 'app-fiche-patient-edit-infermier',
  templateUrl: './fiche-patient-edit-infermier.component.html'
})
export class FichePatientEditInfermierComponent implements OnInit {
    ...
    constructor(private service: FichePatientInfermierService , private consultationService: ConsultationInfermierService, private antecedentService: AntecedentInfermierService, @Inject(PLATFORM_ID) private platformId?) {
    ...
    public prepareEdit() {
        this.item.dateFichePatient = this.service.format(this.item.dateFichePatient);
    }
 ...
 public edit(): void {
   ...
   public async openCreateAntecedent(antecedent: string) {
        const isPermistted = await this.roleService.isPermitted('Antecedent', 'edit');
        if (isPermistted) {
