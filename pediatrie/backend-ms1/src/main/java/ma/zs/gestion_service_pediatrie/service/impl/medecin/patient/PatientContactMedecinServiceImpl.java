...
import java.util.List;
@Service
public class PatientContactMedecinServiceImpl implements PatientContactMedecinService {
    ...
    public List<PatientContact> findByPatientNumDossier(String num) {
    ...
    @Override
    public List<List<PatientContact>> getToBeSavedAndToBeDeleted(
            List<PatientContact> existingContacts, List<PatientContact> newContacts) {
        List<PatientContact> toSave = new ArrayList<>();
        List<PatientContact> toDelete = new ArrayList<>();

        for (PatientContact newContact : newContacts) {
            boolean found = false;
            for (PatientContact existingContact : existingContacts) {
                if (existingContact.getCin().equals(newContact.getCin())) {
                    // Update existing contact
                    existingContact.setAdresse(newContact.getAdresse());
                    existingContact.setEmail(newContact.getEmail());
                    existingContact.setNom(newContact.getNom());
                    existingContact.setPrenom(newContact.getPrenom());
                    existingContact.setTelephone(newContact.getTelephone());
                    existingContact.setRelation(newContact.getRelation());
                    existingContact.setPatient(newContact.getPatient());
