...
@Service
public class RadiologieMedecinServiceImpl implements RadiologieMedecinService {
    ...
    @Override
    public List<List<Radiologie>> getToBeSavedAndToBeDeleted(List<Radiologie> existingContacts, List<Radiologie> newContacts) {
        List<Radiologie> toSave = new ArrayList<>();
        List<Radiologie> toDelete = new ArrayList<>();

        // Find contacts to save
        for (Radiologie newContact : newContacts) {
            boolean found = false;
            for (Radiologie existingContact : existingContacts) {
                if (existingContact.getRef().equals(newContact.getRef())) {
                    // Update existing contact
                    existingContact.setCommentaire(newContact.getCommentaire());
                    existingContact.setDateRadiologie(newContact.getDateRadiologie());
                    existingContact.setImageScann(newContact.getImageScann());
                    existingContact.setTypeImage(newContact.getTypeImage());
