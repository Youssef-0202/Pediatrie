...
import ma.zs.gestion_service_pediatrie.service.facade.medecin.dossie.EpreuveMedecinService ;
import ma.zs.gestion_service_pediatrie.bean.core.dossie.Epreuve ;

import java.util.List;
@Service
public class AnalyseMedicaleMedecinServiceImpl implements AnalyseMedicaleMedecinService {
    ...
    @Override
    public List<List<AnalyseMedicale>> getToBeSavedAndToBeDeleted(List<AnalyseMedicale> existingContacts, List<AnalyseMedicale> newContacts) {
        List<AnalyseMedicale> toSave = new ArrayList<>();
        List<AnalyseMedicale> toDelete = new ArrayList<>();

        // Find contacts to save
        for (AnalyseMedicale newContact : newContacts) {
            boolean found = false;
            for (AnalyseMedicale existingContact : existingContacts) {
                if (existingContact.getRef().equals(newContact.getRef())) {
                    // Update existing contact
                    existingContact.setDateAnalyseMedicale(newContact.getDateAnalyseMedicale());
                    existingContact.setEpreuve(newContact.getEpreuve());
                    existingContact.setValeur(newContact.getValeur());
                    existingContact.setValeurRang(newContact.getValeurRang());
    ...
    @Autowired
    private EpreuveMedecinService epreuveService ;
