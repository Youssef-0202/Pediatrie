...
import ma.zs.gestion_service_pediatrie.service.facade.medecin.dossie.FichePatientMedecinService;
...
import ma.zs.gestion_service_pediatrie.service.facade.medecin.dossie.AntecedentMedecinService ;
import ma.zs.gestion_service_pediatrie.bean.core.dossie.Antecedent ;

import java.util.List;
@Service
public class FichePatientMedecinServiceImpl implements FichePatientMedecinService {
    ...
    @Override
    public List<List<FichePatient>> getToBeSavedAndToBeDeleted(List<FichePatient> existingContacts, List<FichePatient> newContacts) {
        List<FichePatient> toSave = new ArrayList<>();
        List<FichePatient> toDelete = new ArrayList<>();

        // Find contacts to save
        for (FichePatient newContact : newContacts) {
            boolean found = false;
            for (FichePatient existingContact : existingContacts) {
                if (existingContact.getRef().equals(newContact.getRef())) {
                    // Update existing contact
                    existingContact.setAntecedent(newContact.getAntecedent());
                    existingContact.setConclusionFichePatient(newContact.getConclusionFichePatient());
                    existingContact.setDateFichePatient(newContact.getDateFichePatient());
                    existingContact.setAntecedentFamillial(newContact.getAntecedentFamillial());
                    existingContact.setHistoireMaladie(newContact.getHistoireMaladie());
    ...
    @Autowired
    private AntecedentMedecinService antecedentService ;
